// Conversation memory: what the learner is currently talking about.
// ---------------------------------------------------------------------------
...
import { tryContextualNumberFollowup } from '../engine/matchingEngine.js';
import { classifyStockIntent } from '../intent/stockIntent.js';
...
// ---------------------------------------------------------------------------
export const EXPIRE_AFTER_TURNS = 3;

export function noteTurnPassed(context){
  if(!context) return;
  context.turnsSinceEntityTouch = (context.turnsSinceEntityTouch || 0) + 1;
}
export function touchEntityContext(context){
  if(!context) return;
  context.turnsSinceEntityTouch = 0;
}
export function isEntityContextStale(context){
  return !!context && context.activeEntity && context.turnsSinceEntityTouch > EXPIRE_AFTER_TURNS;
}
export function expireEntityContextIfStale(context){
...
// this is the exact phrasing used by the AI's OWN suggested follow-up
// chips ("How has it changed...?" / "איך היא השתנתה..."), so missing this
// class of pronoun meant clicking the app's own suggestion could silently
// fail. Kept as a separate list matched by whole WORD, not substring —
// "it" and "היא"/"הוא" are short enough that a plain .includes() would
...
export function containsWholeWord(norm, word){
  ...
  // substring-collision risk above and JS regex \b's unreliable handling
  // of non-ASCII (Hebrew) word characters.
  return norm.split(/\s+/).includes(word);
}
export const METRIC_IMPLICIT_PRONOUN_KW = [
  'זה גבוה','זה נמוך','זה טוב','זה רע','that high','that low','is that high','is that low'
];
export function looksLikeEntityPronounReference(norm){
  return GENERIC_ENTITY_PRONOUN_KW.some(k => norm.includes(normalizeText(k)))
      || SUBJECT_PRONOUN_KW.some(k => containsWholeWord(norm, k))
      || METRIC_IMPLICIT_PRONOUN_KW.some(k => norm.includes(normalizeText(k)));
}
export function looksLikeMetricImplicitReference(norm){
...
// ---------------------------------------------------------------------------
export const ENTITY_COMPARISON_KW = [
...
export function looksLikeEntityComparison(norm){
...
export function recordActiveEntity(context, company){
...
export function recordActiveMetric(context, facet){
  if(!context) return;
