// Company/ticker resolution, including Hebrew aliases.
// ---------------------------------------------------------------------------
...
export const HEBREW_COMPANY_TRIGGERS = [
...
export const HEBREW_FILLER_WORDS = new Set([
...
export const HEBREW_TO_LATIN_MAP = {
...
export function tickerAliasesSorted(){
...
// A handful of ticker aliases are also ordinary English words that appear
// constantly in unrelated financial vocabulary: "cost" -> Costco, "target"
// -> Target Corp. An unqualified word-boundary match on these fires inside
// phrases like "dollar cost averaging" or "price target [fibonacci]",
// silently hijacking an ordinary conceptual question into one company's
// live-data answer instead of the KB topic it was actually about.
//
// Found empirically, not guessed: a sweep of every KB entry's own keywords
// through the full reply pipeline surfaced these two as real, reproducible
// misfires (see tests/core/tickerAmbiguity.test.ts). Any other single-word
// alias that turns out to collide with ordinary usage belongs in this set
// too, found the same way — by testing against real KB phrasing, not by
// guessing which words look risky.
const AMBIGUOUS_ALIASES = new Set(['cost', 'target']);

// A small set of cues that make "this message is actually asking about a
// specific company" explicit, in either language. Deliberately narrow:
// widening it defeats the point, which is requiring a REAL signal.
const COMPANY_SIGNAL_CUES = [
  'stock', 'share', 'shares', 'ticker', 'symbol', 'quote', 'dividend', 'earnings',
  'market cap', 'p/e', 'pe ratio', 'stock price',
  'מניה', 'מניית', 'מחיר', 'שווי', 'מכפיל', 'דיבידנד', 'טיקר', 'סימול'
];

function hasCompanySignal(norm) {
  return COMPANY_SIGNAL_CUES.some((cue) => norm.includes(normalizeText(cue)));
}
...
export function resolveTicker(norm){
  ...
  for(const { alias, entry } of tickerAliasesSorted()){
    ...
    if(AMBIGUOUS_ALIASES.has(alias)){
      ...
      // company, not the English word.
      const isWholeMessage = norm === alias;
      if(!isWholeMessage && !hasCompanySignal(norm)) continue;
    }
    return entry;
  }
  return null;
