// Technical indicator maths. Pure — array in, number out.
// ---------------------------------------------------------------------------
...
export function mdSma(values, period){
...
export function volatilityPct(values, period){
  period = period || 20;
  if(values.length < period + 1) return null;
  const slice = values.slice(-(period+1));
  const returns = [];
...
export function fiftyTwoWeek(history){
...
export function describeTrend(closes, he){
