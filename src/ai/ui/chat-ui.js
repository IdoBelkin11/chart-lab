...
// Explicit "start over" — distinct from closing the overlay. Requested so
// there's always a way back to the example-questions home screen without
// losing your place on the site (closeAiChat) OR having to type your way
// back to a clean slate. Clears history/context and restores the empty
// state exactly as a freshly-opened chat would look.
function resetAiChat(){
  aiHistory.length = 0;
  lastTopicId = null;
  aiConversationContext.activeEntity = null;
  aiConversationContext.previousEntity = null;
  aiConversationContext.activeMetric = null;
  aiConversationContext.turnsSinceEntityTouch = 0;
...
// Browsing categories/topics used to append a new chip-row to the chat log
// every time you drilled in — so scrolling up during a browse session
// showed the categories list still sitting there above whatever topic list
// you'd since drilled into, unrelated to the actual conversation. Browsing
// is now a single, persistent panel updated IN PLACE (category list → a
// specific category's topics → back to categories), never stacking a new
// row in the scrollback. It's replaced by a real chat message only once an
// actual topic is picked (askAiTopic already handles that normally).
function getOrCreateBrowsePanel(){
  let row = document.getElementById('ai-browse-panel');
  if(row) return row;
  const wrap = document.getElementById('ai-messages');
  row = document.createElement('div');
  row.id = 'ai-browse-panel';
  row.className = 'ai-msg assistant';
  wrap.appendChild(row);
  return row;
}
function removeBrowsePanel(){
  const row = document.getElementById('ai-browse-panel');
  if(row) row.remove();
}
function renderChipsInto(container, buttons){
  container.innerHTML = '';
  const chipWrap = document.createElement('div');
  chipWrap.className = 'ai-examples';
  chipWrap.style.margin = '0 0 4px 0';
...
function renderTopicCategories(){
  document.getElementById('ai-empty').style.display = 'none';
  const panel = getOrCreateBrowsePanel();
  renderChipsInto(panel, CATEGORY_ORDER.map(cat => ({
...
function renderTopicList(cat){
  ...
  const panel = getOrCreateBrowsePanel();
  const backLabel = lang === 'he' ? '← כל הקטגוריות' : '← All categories';
  renderChipsInto(panel, [
    { label: backLabel, onClick: function(){ renderTopicCategories(); } },
    ...topics.map(entry => ({
      label: pickLabelTerm(entry.kw, lang === 'he'),
      onClick: function(){ removeBrowsePanel(); askAiTopic(entry.id); }
    }))
  ]);
}
...
async function sendAiMessage(){
  ...
  if(!text) return;
  removeBrowsePanel(); // typing a real question ends any in-progress browse session
  inp.value = '';
