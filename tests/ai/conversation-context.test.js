...
(async () => {
  ...
  await describe('Regression: an unrecognized pronoun question falls through rather than forcing a snapshot', async (t) => {
    // A generic pronoun reference ("her X?") that names something this
    // layer has no facet for at all must not invent an answer — it should
    // fall through to normal KB handling (which may itself find something,
    // or reach the honest fallback), never silently substitute a snapshot.
    const engine = loadEngine(createTwelveDataMock());
    const ctx = engine.createConversationContext();
    await engine.generateAiReply('ספר לי על NVIDIA', 'he', null, ctx);
    const r = await engine.generateAiReply('מה איתה?', 'he', null, ctx); // "what about her?" — names nothing specific
    t.check('does not force a price-snapshot answer for an unresolvable pronoun question', !/^\S+.*: \$\d/.test(r.text));
  });
