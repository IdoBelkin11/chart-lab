  ...
  if (url.includes("/time_series")) {
    ...
    for (let i = 0; i < 260; i++) {
      ...
      values.push({
        ...
        open: (close - 0.3).toFixed(2),
        high: (close + 0.5).toFixed(2),
        low: (close - 0.6).toFixed(2),
    ...
    return Promise.resolve({
      ok: true,
      json: async () => ({ meta: {}, values: values.reverse() }),
    });
  }
...
async function loadPage(fetchMock) {
  if (!fs.existsSync(DIST_PATH)) {
    throw new Error(
      "dist/index.html not found — run `node scripts/build.js` first (npm test does this automatically via pretest).",
    );
  }
